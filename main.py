...
class FinnDealFinderApp(ctk.CTk):
    ...
    CATEGORIES = {
        'Torget (Marketplace)': {
            # UPDATED: new recommerce endpoint
            'url_base': 'https://www.finn.no/recommerce/forsale/search',
            'subcategories': {
                'Alle kategorier': '',
                'Antikviteter og kunst': 'category=1.60',
                'Dyr og utstyr': 'category=1.61',
                # UPDATED: correct top-level Elektronikk og hvitevarer category
                'Elektronikk og hvitevarer': 'category=0.93',
    ...
    def _run_search(self, params: dict):
        ...
        try:
            ...
            # Python 3.11: capture e in default argument so closure is safe
    ...
    def _display_results(self, results: dict):
        ...
        hot_deals = [
            ...
            if item.get('deal_score', 0) >= self.deal_threshold_var.get()
        ]
    ...
    def _update_statistics(self, results: dict):
        ...
        hot_deals = len([
            i for i in items
            if i.get('deal_score', 0) >= self.deal_threshold_var.get()
        ])
